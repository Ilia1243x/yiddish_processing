...
def lemmatize_text(file_path, new_path, d_f): #путь к файлу, путь для нового файла, словарь форм, словарь счетчик
    text = None
    ...
    for line in text:
        ...
        for i in range (len(split_line)):
            if (split_line[i] == '.' or split_line[i] == '?' or split_line[i] == '!'):
                ...
                lemm_line += " "
                continue
            #split_line[i] = split_line[i].replace('כֿ','כ') #ИДДИШ
            #split_line[i] = split_line[i].replace('עֶ','ע') #ИДДИШ
            #split_line[i] = split_line[i].replace('עֶָ','ע') #ИДДИШ
            split_line[i] = split_line[i].replace('-','') #SPOT ON
            #все в виво
            ...
            if split_line[i] in preps or split_line[i] in merged_preps:
                lemm_line += split_line[i]
                lemm_line += " "
                continue #если предлог то мы так и добавляем
            if split_line[i] in conjunctions:
                lemm_line += split_line[i]
                lemm_line += " "
                continue #если союз то то мы так и добавляем
            ...
            if split_line[i] in articles:
                lemm_line += 'ןןןן'
                lemm_line += " "
                continue #я пока не понял че делать с артиклями, либо мусор либо токены
            if (len(split_line[i]) <= 2):
                #не артикли, не предлоги, не союзы, не местоимения но 1-2 буквы => мусор
                continue
            if split_line[i] in irr_verbs.values(): #то есть это инфинитив непр глагола
                if (split_line[i] == 'וועלן'):
                    continue
            ...
            if (i > 0 and (split_line[i-1] in single_articles or split_line[i-1] in merged_preps)):
                ...
                #итого три варианта что добавить
                if (split_line[i] in nouns):
                    lemm_line += split_line[i]
                    lemm_line += " "
                    continue
                if (split_line[i] in adjectives):
                    lemm_line += split_line[i]
                    lemm_line += " "
                    continue
                if (cut_word in adjectives):
                    lemm_line += cut_word
                    lemm_line += " "
                    continue
                if (lemm in adjectives):
                    lemm_line += lemm
                    lemm_line += " "
                    continue
                c1 = len(d_f.get(split_line[i], set()))
                c2 = len(d_f.get(cut_word, set()))
                c3 = len(d_f.get(lemm, set()))
                if (c1 == max(c1,c2,c3)):
                    lemm_line += split_line[i]
                    lemm_line += " "
                    continue
            ...
            if (i > 0 and split_line[i-1] in articles):
                ...
                if (lemm in adjectives):
                    ...
                    continue
                c1 = len(d_f.get(split_line[i], set()))
                c2 = len(d_f.get(noun, set()))
                c3 = len(d_f.get(cut_word, set()))
                c4 = len(d_f.get(lemm, set()))
            ...
            if (split_line[i] in help_words):
                #многпрошедшее время
                #где-то впереди инфинитив, но это слово мы не берем
                continue
            if (split_line[i] in have):
                ...
                if (i < len(split_line)-1 and split_line[i+1] in articles):
                    ...
                    #где-то впереди партицип, но это слово мы не берем
                    continue
            ...
            if (split_line[i] in future):
                #будущее время, впереди где-то инфинитив, но это слово не берем
                continue
            if (split_line[i] == special):
                #предпрошедшее время, где-то впереди партицип, но это слово не берем
                continue
            ...
            cut_word = cut_adj(split_line[i]) #убрали родовое окончание 
            lemm = normalize_adj(cut_word) #прилагательное в сравнительной форме
            adv = normalize_adj(split_line[i]) #наречие в сравнительной форме
            noun = normalize_noun(split_line[i])
            if (verb in verbs):
                lemm_line += verb
                lemm_line += " "
            ...
            if (noun in nouns):
                ...
                continue
            c1 = len(d_f.get(split_line[i], set()))
            c2 = len(d_f.get(verb,set()))
            c3 = len(d_f.get(cut_word, set()))
            c4 = len(d_f.get(lemm, set()))
            c5 = len(d_f.get(noun, set()))
            c6 = len(d_f.get(adv, set()))
...
def lemmatizer(dir_root, new_root): #путь
    d_f = dict() #слорваь счетчик
    with open("forms.json", "r", encoding="utf-8") as dict_json:
        d_f = json.load(dict_json)
    print('ready')
    ...
    for i in range(len(files)):
        ...
        lemmatize_text(file_path, new_path, d_f)
        #return

#Так запускал я, но поменяю для удобства
#path_old = 'C:/Users/Ilia2024/Desktop/yiddish/after/'
#path_new = 'C:/Users/Ilia2024/Desktop/yiddish/res/'

path_old = 'after/'
path_new = 'res/'

lemmatizer(path_old, path_new)
